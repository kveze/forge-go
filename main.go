...
func generateHandler(w http.ResponseWriter, r *http.Request) {
	...
	systemPrompt := `Ты профессиональный фитнес-тренер с 10-летним опытом.
Твоя задача — создавать безопасные и эффективные планы тренировок.

ПРАВИЛА:
1. Используй ТОЛЬКО реальные упражнения (отжимания, приседания, тяги и т.д.)
2. НЕ придумывай несуществующие упражнения
3. Учитывай уровень подготовки и доступное оборудование
4. НЕ нагружай одну группу мышц два дня подряд
5. Верни ТОЛЬКО валидный JSON без markdown, без пояснений
...
func buildSystemPrompt(userData map[string]interface{}, plan interface{}) string {
    ...
    return fmt.Sprintf(`Ты персональный AI тренер FORGE. Строгий, немного тайлер дерден и девидгогинс, честный, поддерживающий. Говоришь кратко. - Если вопрос не связан с фитнесом, тренировками, питанием или восстановлением — вежливо скажи что ты специализируешься только на фитнесе и верни разговор к теме
...
ИЗМЕНЕНИЕ ПЛАНА:
Если пользователь говорит что план слишком лёгкий или тяжёлый — ты ОБЯЗАН сгенерировать обновлённый план.
Сначала напиши 1-2 предложения объяснения что изменил, затем верни новый план в формате:

ПЛАН_СТАРТ
{"week_plan": [...], "goal": "...", "level": "..."}
ПЛАН_КОНЕЦ
